...
// Always metres, never a mm/m mix. Sellers enter sizes in mm (the precise
// unit a sign gets fabricated to), but every buyer-facing surface shows one
// consistent unit — switching between "2.0m" and "800mm" on the same page
// made sizes hard to compare at a glance.
export function formatMm(mm) {
  const m = (Number(mm) || 0) / 1000;
  // Trim a trailing ".0" so 2000mm reads "2m", not "2.0m", while 2500mm
  // still reads "2.5m".
  return `${parseFloat(m.toFixed(2))}m`;
}

// Every price a seller enters and every price shown on a listing is
// GST-exclusive — Frontage is a B2B marketplace, so quoting ex-GST is the
// convention buyers expect. GST is added once, at checkout, on the total.
export const GST_RATE = 0.1;
export const GST_LABEL = "excl. GST";

export function gstOn(amountExGst) {
  return round2(amountExGst * GST_RATE);
}

export function withGst(amountExGst) {
  return round2(amountExGst * (1 + GST_RATE));
}

// Money math on floats drifts (0.1 + 0.2); every derived total gets rounded
// to cents at the point it's computed so the displayed figures always add up
// to the charged figure exactly.
...
const EYES_BASE_BY_CATEGORY = { gym: 80, cafe: 300, office: 150, studio: 100, retail: 400, residential: 250, other: 150 };
