...
// Password reset — token is single-use (cleared by clearPasswordResetToken
// once the password is actually changed) and time-limited (expires_at is an
// epoch-ms cutoff, same convention as sessions.expires_at). A token past its
// expiry is treated exactly like no token at all.
export async function getUserByResetToken(token) {
  if (!token) return null;
  const { rows } = await pool.query(`SELECT * FROM users WHERE password_reset_token = $1`, [token]);
  const user = rowToObj(rows[0]);
  if (!user) return null;
  if (!user.passwordResetExpiresAt || user.passwordResetExpiresAt < Date.now()) return null;
  return user;
}
export async function setPasswordResetToken(id, token, expiresAt) {
  return updateRow("users", id, { passwordResetToken: token, passwordResetExpiresAt: expiresAt });
}
export async function clearPasswordResetToken(id) {
  return updateRow("users", id, { passwordResetToken: null, passwordResetExpiresAt: null });
}
