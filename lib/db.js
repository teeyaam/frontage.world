...
import { estimateJobFee } from "./format.js";
...
export async function createBookingBundle({ listing, buyerId, term, signature, paymentIntentId = null }) {
  ...
  const total = monthlyRate * term;
  const feeRate = 0.15;
  const feeAmount = Math.round(total * feeRate * 100) / 100;
  const payoutAmount = Math.round((total - feeAmount) * 100) / 100;
  ...
  try {
    ...
    await client.query(
      `INSERT INTO payments (id, seq, booking_id, total_amount, fee_amount, payout_amount, status, paid_at, payout_status, payout_released_at, payout_blocked_reason, stripe_payment_intent_id)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'held',NULL,NULL,$9)`,
      [`PM-${1000 + pSeq}`, pSeq, booking.id, total, feeAmount, payoutAmount, paymentResult.status, paymentResult.paidAt, paymentIntentId]
    );
