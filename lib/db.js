...
// DATE (no type parser registered here previously) defaults to node-pg's own
// parser, which builds a *local-timezone* `new Date(year, month-1, day)` —
// round-tripping that through .toISOString() elsewhere in the app (e.g.
// lib/flightCalendar.js) shifts the calendar date by a day whenever the
// server's local timezone isn't UTC. Postgres already sends DATE as a plain
// "YYYY-MM-DD" string over the wire; keep it exactly that, with no Date
// object in between, the same way every other date-shaped value in this
// app (campaign_start_date, permit_expiry, etc.) is meant to be handled.
...
export async function createBookingBundle({ listing, buyerId, term, signature, paymentIntentId = null, campaignStartDate = null, campaignEndDate = null }) {
  ...
  try {
    ...
    const bookingRow = await client.query(
      `INSERT INTO bookings (id, seq, listing_id, buyer_id, seller_id, term, monthly_rate, status, lease_start_date, auto_renew, created_at, campaign_start_date, campaign_end_date)
       VALUES ($1,$2,$3,$4,$5,$6,$7,'signed',NULL,TRUE,$8,$9,$10) RETURNING *`,
      [bookingId, bSeq, listing.id, buyerId, listing.ownerId, term, monthlyRate, nowIso, campaignStartDate, campaignEndDate]
    );
...
// 03-AD-SPACE-TRANSLATION.md §5 §8) — see routes/pages.js#cartPage and
// routes/api.js#checkoutCartHandler for the flow this backs.
export async function addToCart({ buyerId, listingId, term, campaignStartDate }) {
  // One line per listing per cart — re-adding the same listing updates the
  // term/date instead of piling up duplicate rows.
  const { rows } = await pool.query(`SELECT * FROM cart_items WHERE buyer_id = $1 AND listing_id = $2`, [buyerId, listingId]);
  if (rows.length) {
    return updateRow("cart_items", rows[0].id, { term, campaignStartDate });
  }
  const { id, __seq } = await nextId("cart_items_seq", "CI");
  return insertRow("cart_items", { id, __seq, buyerId, listingId, term, campaignStartDate });
}
export async function getCartForBuyer(buyerId) {
  const { rows } = await pool.query(`SELECT * FROM cart_items WHERE buyer_id = $1 ORDER BY seq ASC`, [buyerId]);
  return rows.map(rowToObj);
}
export async function removeCartItem(id, buyerId) {
  await pool.query(`DELETE FROM cart_items WHERE id = $1 AND buyer_id = $2`, [id, buyerId]);
}
export async function clearCartForBuyer(buyerId) {
  await pool.query(`DELETE FROM cart_items WHERE buyer_id = $1`, [buyerId]);
}

// Checks out every item at once: one orders row, then the same
// booking/contract/payment/job-order bundle createBookingBundle creates for
// a single listing, repeated per item, all in one transaction, all tagged
// with the shared order_id. Charging (or verifying an already-charged
// PaymentIntent for the combined total) happens by the caller before this
// is called, same convention as createBookingBundle.
export async function createOrderWithBookings({ buyerId, items, signature, paymentIntentId = null, totalAmount }) {
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    const nowIso = new Date().toISOString();

    const oSeq = (await client.query(`SELECT nextval('orders_seq') AS n`)).rows[0].n;
    const orderId = `OR-${1000 + oSeq}`;
    await client.query(`INSERT INTO orders (id, seq, buyer_id, total_amount, stripe_payment_intent_id, created_at) VALUES ($1,$2,$3,$4,$5,$6)`, [
      orderId,
      oSeq,
      buyerId,
      totalAmount,
    ...
    for (const item of items) {
      const { listing, term, campaignStartDate, campaignEndDate } = item;
      const monthlyRate = listing.price;
      const subtotal = round2(monthlyRate * term);
      const gstAmount = gstOn(subtotal);
      const total = round2(subtotal + gstAmount);
      const feeAmount = round2(subtotal * 0.15);
      const payoutAmount = round2(subtotal - feeAmount);

      const bSeq = (await client.query(`SELECT nextval('bookings_seq') AS n`)).rows[0].n;
      const bookingId = `BK-${1000 + bSeq}`;
      const bookingRow = await client.query(
        `INSERT INTO bookings (id, seq, listing_id, buyer_id, seller_id, term, monthly_rate, status, lease_start_date, auto_renew, created_at, campaign_start_date, campaign_end_date, order_id)
         VALUES ($1,$2,$3,$4,$5,$6,$7,'signed',NULL,TRUE,$8,$9,$10,$11) RETURNING *`,
        [bookingId, bSeq, listing.id, buyerId, listing.ownerId, term, monthlyRate, nowIso, campaignStartDate, campaignEndDate, orderId]
      );
      const booking = rowToObj(bookingRow.rows[0]);
      ...
      await client.query(
        `INSERT INTO payments (id, seq, booking_id, total_amount, subtotal_amount, gst_amount, fee_amount, payout_amount, status, paid_at, payout_status, payout_released_at, payout_blocked_reason, stripe_payment_intent_id)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'paid',$9,'held',NULL,NULL,$10)`,
        [`PM-${1000 + pSeq}`, pSeq, booking.id, total, subtotal, gstAmount, feeAmount, payoutAmount, nowIso, paymentIntentId]
      );
      ...
      await client.query(`UPDATE listings SET status = 'leased' WHERE id = $1`, [listing.id]);
      bookings.push({ booking, jobOrder: rowToObj(jobOrderRow.rows[0]) });
    }
...
export async function getOrderById(id) {
  return getRowById("orders", id);
}
export async function getBookingsForOrder(orderId) {
  const { rows } = await pool.query(`SELECT * FROM bookings WHERE order_id = $1 ORDER BY seq ASC`, [orderId]);
  return rows.map(rowToObj);
}
...
// ---------- Cancellations (tiered fee + make-good — lib/cancellation.js) ----------
export async function createCancellation({ bookingId, tier, feeAmount, reason }) {
  const { id, __seq } = await nextId("cancellations_seq", "CN");
  return insertRow("cancellations", { id, __seq, bookingId, tier, feeAmount, reason: reason || null, makeGoodType: "none", makeGoodDetails: null });
}
export async function getCancellationsForBooking(bookingId) {
  const { rows } = await pool.query(`SELECT * FROM cancellations WHERE booking_id = $1 ORDER BY seq DESC`, [bookingId]);
  return rows.map(rowToObj);
}
export async function resolveCancellationMakeGood(id, { makeGoodType, makeGoodDetails }) {
  return updateRow("cancellations", id, { makeGoodType, makeGoodDetails: makeGoodDetails || null, resolvedAt: new Date().toISOString() });
}
