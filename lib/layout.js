...
import { adsHeadScript } from "./ads.js";
...
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&family=IBM+Plex+Mono:wght@500;600&family=Caveat:wght@600&display=swap" rel="stylesheet" />
...
<footer class="site-footer">
  ...
  <div>© ${new Date().getFullYear()} Frontage. Advertising space, direct from the people who own it.</div>
</footer>
...
function brandMark(tagline) {
  return `<a href="/" class="brand">
      ...
      <span>FRONTAGE</span>
      ${tagline ? `<span class="brand-tagline">${escapeHtml(tagline)}</span>` : ""}
    </a>`;
}
...
export async function layout({ title, activeNav = "", user = null, body = "", flash = "" }) {
  const nav = [navLink("/", "Browse", activeNav === "browse"), staffDropdown(activeNav, user)].filter(Boolean).join("");
  ...
  // "Switch to listing a space" — a deliberate, distinctly-styled mode
  // switch (pill button, not a plain nav link) rather than blending in,
  // matching Airbnb's persistent "Switch to hosting" affordance even
  // though Frontage keeps one unified account underneath. A first-time
  // seller (no listings yet) goes via /sell/welcome's intro splash first;
  // once you have at least one listing it goes straight to /sell/new and
  // relabels itself, same as Airbnb's own switch does once you're a host.
  const listingCount = user ? (await db.getListingsByOwner(user.id)).length : 0;
  const sellSwitch = user
    ? `<a href="${listingCount > 0 ? "/sell/new" : "/sell/welcome"}" class="btn-pill btn btn-sm${activeNav === "sell" ? " is-active" : ""}">${listingCount > 0 ? "My listings" : "Switch to listing a space"}</a>`
    : "";
  ...
  const headerInner = `${brandMark("List your wall, window, or fence — or find the spot where your business can be really seen.")}
