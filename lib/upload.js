...
// Campaign artwork (Phase 4 content-approval workflow) — public like listing
// photos (the seller/owner needs to view it to approve/reject), image or PDF
// like a contractor doc (artwork is often supplied as a print-ready PDF).
...
if (isS3Configured()) {
  ...
  uploadArtwork = multer({
    storage: multerS3({
      s3,
      bucket: process.env.S3_BUCKET,
      key: (req, file, cb) => cb(null, `artwork/${uniqueName(file)}`),
      contentType: multerS3.AUTO_CONTENT_TYPE,
    }),
    limits: { fileSize: DOC_FILE_SIZE_LIMIT, files: 1 },
    fileFilter: (req, file, cb) => cb(null, file.mimetype.startsWith("image/") || file.mimetype === "application/pdf"),
  }).single("artwork");
...
export { uploadListingPhotos, uploadContractorDocs, uploadArtwork, getContractorDocUrl };

// Resolves the public URL for an uploaded artwork file, regardless of
// storage backend — mirrors photoPublicUrl below.
export function artworkPublicUrl(file) {
  if (file.key) {
    const base = (process.env.S3_PUBLIC_BASE_URL || "").replace(/\/$/, "");
    return `${base}/${file.key}`;
  }
  return `/uploads/artwork/${file.filename}`;
}
